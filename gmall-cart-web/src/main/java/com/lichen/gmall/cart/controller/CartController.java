package com.lichen.gmall.cart.controller;
...
import com.lichen.gmall.util.CookieUtil;
...
public class CartController {
    ...
    public String checkCart(HttpServletRequest request, HttpServletResponse response, CartInfo cartInfo, ModelMap map) {
        ...
        if (StringUtils.isBlank(userId)) {
            //取cookie中的数据
            cartInfos = getCartInfos4Cookie(request, cartInfos);
        } else {
            //取缓存中的数据
    ...
    public String cartList(HttpServletRequest request, ModelMap map) {
        ...
        if (StringUtils.isBlank(userId)) {
            cartInfos = getCartInfos4Cookie(request, new ArrayList<CartInfo>());
        } else {
            cartInfos = cartService.getCartCache(userId);
        }
     ...
     * 从Cookie中获取购物车数据
     * @param request
     * @param cartInfos    可以是从缓存里来的
     * @return
     */
    private List<CartInfo> getCartInfos4Cookie(HttpServletRequest request, List<CartInfo> cartInfos) {
        //取cookie中的数据
        String cartListCookie = CookieUtil.getCookieValue(request, "cartListCookie", true);
        if (StringUtils.isNotBlank(cartListCookie)) {
            cartInfos = JSON.parseArray(cartListCookie, CartInfo.class);
