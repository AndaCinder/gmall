package com.lichen.gmall.passport.controller;
...
import com.lichen.gmall.util.PassportUtil;
import com.lichen.gmall.service.UserService;
import com.lichen.gmall.util.JwtUtil;
...
public class PassportController {
    ...
    public String login(HttpServletRequest request, HttpServletResponse response, UserInfo userInfo) {
        ...
        if (user == null) {
            ...
            String token = JwtUtil.encode("AndaCinder", map, PassportUtil.getRequestAddr(request));
    ...
    public String verify(String token, String salt) {
        Map userMap = null;
        try {
            userMap = JwtUtil.decode("AndaCinder", token, salt);
        } catch (Exception e) {
            return "fail";
        }
        if (userMap != null) {
